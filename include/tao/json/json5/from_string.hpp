...
#ifndef TAOCPP_JSON_INCLUDE_JSON5_FROM_STRING_HPP
#define TAOCPP_JSON_INCLUDE_JSON5_FROM_STRING_HPP

#include <cstddef>
#include <string>
#include <utility>

#include "../events/json5/from_string.hpp"
   ...
   namespace json
   {
      namespace json5
      {
         ...
         basic_value< Traits > from_string( const char* data, const std::size_t size, const char* source = nullptr, const std::size_t byte = 0, const std::size_t line = 1, const std::size_t column = 0 )
         {
            ...
            events::json5::from_string( data, size, consumer, source, byte, line, column );
         ...
         {
            return json5::from_string( data.data(), data.size(), source, byte, line, column );
         }
         ...
         {
            return json5::from_string( data.data(), data.size(), source.c_str(), byte, line, column );
         }
         ...
         {
            inline value operator"" _json5( const char* data, const std::size_t size )
            {
               return json5::from_string( data, size, "literal" );
            }
