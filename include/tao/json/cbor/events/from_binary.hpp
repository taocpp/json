...
namespace tao::json::cbor::events
{
   ...
   void from_binary( Consumer& consumer, const void* data, const std::size_t size, const char* source = nullptr, const std::size_t byte = 0, const std::size_t line = 1, const std::size_t byte_in_line = 0 )
   {
      ...
      pegtl::memory_input< pegtl::tracking_mode::lazy, pegtl::eol::lf_crlf, const char* > in( cast, cast + size, source ? source : "tao::json::cbor::events::from_binary", byte, line, byte_in_line );
   ...
   {
      cbor::events::from_binary( consumer, data, size, source.c_str(), byte, line, byte_in_line );
   }
