...
#define TAOCPP_JSON_PEGTL_INCLUDE_INTERNAL_FILE_READER_HPP
      ...
      namespace internal
      {
         ...
         public:
            ...
            std::size_t size() const
            {
               ...
               if( std::fseek( m_file.get(), 0, SEEK_END ) != 0 ) {
                  TAOCPP_JSON_PEGTL_THROW_INPUT_ERROR( "unable to fseek() to end of file " << m_source );  // LCOV_EXCL_LINE
               }
               errno = 0;
               const auto s = std::ftell( m_file.get() );
               if( s < 0 ) {
                  TAOCPP_JSON_PEGTL_THROW_INPUT_ERROR( "unable to ftell() file size of file " << m_source );  // LCOV_EXCL_LINE
               }
               errno = 0;
               if( std::fseek( m_file.get(), 0, SEEK_SET ) != 0 ) {
                  TAOCPP_JSON_PEGTL_THROW_INPUT_ERROR( "unable to fseek() to beginning of file " << m_source );  // LCOV_EXCL_LINE
               }
               return std::size_t( s );
            }
         ...
         private:
            ...
            std::FILE* open() const
            {
               ...
               TAOCPP_JSON_PEGTL_THROW_INPUT_ERROR( "unable to fopen() file " << m_source << " for reading" );
            }
         };
